...
import {
  ...
  getChatGptMcpWindowTitlePatterns,
...
function buildPowerShellPrelude(options?: {
  ...
  return [
    ...
    '    Start-Sleep -Milliseconds 80',
    '  }',
    '  [void][NativeDesktopAutomation]::ShowWindow($Handle, 5)',
...
export class WindowsDesktopAutomation {
  ...
  private async approveChromeWindowPrompt(
    ...
    do {
      ...
      [
        ...
        '$windowTitlePatterns = @($payload.windowTitlePatterns | ForEach-Object { "$_".ToLowerInvariant() })',
        ...
        '  return ([regex]::Replace($normalizedText, "[^\\p{L}\\p{Nd}]+", ""))',
        '}',
        ...
        '    $normalizedFragment = Normalize-UiText $fragment',
        '    $compactFragment = Compact-UiText $fragment',
        '    if ($normalizedText.Contains($normalizedFragment)) { return $true }',
        '    if (-not [string]::IsNullOrWhiteSpace($compactFragment) -and $compactText.Contains($compactFragment)) { return $true }',
        '  }',
        '  return $false',
        '}',
        ...
        '    if (-not [string]::IsNullOrWhiteSpace($pattern) -and $normalizedName.Contains((Normalize-UiText $pattern))) { return -1 }',
        '  }',
        '  foreach ($pattern in $rejectButtonPatterns) {',
        '    if (-not [string]::IsNullOrWhiteSpace($pattern) -and $normalizedName.Contains((Normalize-UiText $pattern))) { return -1 }',
        '  }',
        '  if ($allowLabels -contains $normalizedName) { return 1000 }',
        '  if ($expectedPrimaryActionPatterns.Count -gt 0 -and (Matches-NormalizedFragment $Name $expectedPrimaryActionPatterns)) { return 850 }',
        ...
        '    $windowTitleDetected = ($windowTitlePatterns.Count -eq 0) -or (Matches-NormalizedFragment $window.title $windowTitlePatterns)',
        '    if ($windowTitleDetected) {',
        '      Activate-WindowHandle $windowHandle',
        '      Start-Sleep -Milliseconds 120',
        '    }',
        '    $root = [System.Windows.Automation.AutomationElement]::FromHandle($windowHandle)',
        '    if ($null -eq $root) { continue }',
        '    $scanItems = Get-AutomationScanItems $root 3000',
        '  } catch {',
        '    continue',
        '  }',
        ...
        '  $expectedPositiveButtonDetected = $true',
        ...
        '  }',
        '  $appOrWindowDetected = $appDetected -or $windowTitleDetected',
        '  if (-not (($promptDetected -or ($positiveButtonDetected -and $negativeButtonDetected)) -and $expectedPromptDetected -and $expectedPositiveButtonDetected -and $appOrWindowDetected -and $contextDetected)) { continue }',
  ...
  async approveChatGptMcpPrompt(): Promise<unknown> {
    return await this.approveChromeWindowPrompt({
      ...
      windowTitlePatterns: getChatGptMcpWindowTitlePatterns(),
